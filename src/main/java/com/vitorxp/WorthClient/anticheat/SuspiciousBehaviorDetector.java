package com.vitorxp.WorthClient.anticheat;
...
public class SuspiciousBehaviorDetector {
    ...
    private void sendWebhook(String title, String content) {
        ...
        new Thread(() -> {
            try {
                ...
                connection.setRequestProperty("User-Agent", "WorthClient");
                ...
                String json = String.format(
                        "{"
                                + "\"username\": \"WorthClient - AntiCheat\","
                                + "\"embeds\": [{"
                                + "  \"title\": \"%s\","
                                + "  \"description\": \"%s\\n**Ping**: %d\","
                                + "  \"color\": 16711680"
        ...
        if (mc.gameSettings.keyBindAttack.isKeyDown()) {
            if (now - lastAttackTime > 200) {
                ...
                if (delay >= 100 && delay < 350) {
                    ...
                    if (suspiciousSprintHits >= 6 && now - lastSprintExploitAlert > 12000) {
    ...
    private void detectNoSlow() {
        if (mc.thePlayer.isUsingItem() && mc.thePlayer.moveForward > 0.2f) {
